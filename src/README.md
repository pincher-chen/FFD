...
!
```
包括bond/angle/dihedrals/improper,分别表是键/角/二面角/不当二面角四个部分来约束一个分子的几何形状，这里需要知道该体系共有6种原子类型（C3/C4/H1/F1/N3/Cu2）,然后是6种键的类型，9种角的类型，12种二面角类型以及0种不当二面角类型，后面要的的便是对于任意规模的体系，能够根据输入文件的坐标以及键连信息，分别将上述的键/角/二面角/不当二面角进行编号和归类，然后输出为给定的格式（lammps/DL_POLY/GROMACS等）。
